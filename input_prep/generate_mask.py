...
def generate_pixel_mask_from_region_file(image_file,reg_file):
    ...
    if not (exists(image_file) and isfile(image_file)):
        return {'status_code':-1,'err_msg':'File {0} does not exist '.format(image_file)}
    
    if not (exists(reg_file) and isfile(reg_file)):
        return {'status_code':-1,'err_msg':'File {0} does not exist '.format(reg_file)}
    ...
    dmcopy(infile='{0}[sky=region({1})][opt full]'.format(temp_file,reg_file)
            ,outfile=mask_file,
            clobber=True)
